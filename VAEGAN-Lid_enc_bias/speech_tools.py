...
def load_wavs_random_sample(wav_dir, sr, num_samples):

    # load random data
    wavs = list()

    for path, dirs, files in os.walk(wav_dir):
        # print('p:%s,d:%s,f:%s'%(path, dirs, files))
        num_files = len(files)
        if num_files != 0:
            # print('file not empty')
            file_id = np.arange(num_files)
            np.random.shuffle(file_id)
            # print(len(files))
            assert num_files > num_samples, 'p:%s,d:%s,f:%s'%(path,dirs,files)
            print(path)
...
def get_file_paths_random_sample(dir, num_samples):
...
def world_encode_data(wavs, fs, frame_period=5.0, coded_dim=24):
    ...
    # for wav in tqdm(wavs):
        print('Encoding %s data, length:%s'%(count, len(wav)))
        count+=1
